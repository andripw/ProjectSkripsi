 ...
 *
 * Nodes will select a random direction and speed. They travel until either: 1)
 * a set distance is traveled OR 2) a fixed time has passed
 *
 * For example, the user will specify either "-t 60" for a time interval of 60
 * seconds or "-d 45" for a distance of 45 units.
 *
 * Nodes will reflect/bounce off walls and continue traveling should they come
 * into contact.
 *
 */
public class RandomWalkTest extends RandomSpeedBase {
    ...
    public RandomWalkTest(int nodes, double x, double y, double duration, double ignore, long randomSeed, double minspeed, double maxspeed, char mode, double modeDelta) {
    ...
    public RandomWalkTest(String[] args) {
        go(args);
    }
    ...
    public RandomWalkTest(String args[], Scenario _pre, Integer _transitionMode) {
            ...
            while (t < parameterData.duration) {
                ...
                for (double key : dstList.keySet()) {
                    ...
                    if (!parameterData.nodes[i].add(t, dst)) {
                        throw new RuntimeException(getInfo().name + ".go: error while adding waypoint (2t)");
                    }
                }
    ...
    private Position generatePosition(Position currentPos) {
        Position pos = null;
        double rand = Math.random();
        switch (cekPosition(currentPos).areaID) {
            case 0:
                if (rand < 0.5) {
    ...
    private Area cekPosition(Position p) {
        for (Area a : area) {
            if (a.isInsideArea(p)) {
                return a;
            }
    ...
    private void initArea() {
        area.add(new Area(400, 500, 450, 550, 0));
        area.add(new Area(10, 100, 450, 550, 1));
        area.add(new Area(400, 500, 60, 150, 2));
        area.add(new Area(10, 100, 60, 150, 3));
        area.add(new Area(200, 300, 200, 300, 4));
     ...
     * @returnThis function will check if the node is attempting to move outside
     * of the simulation space. It will return a SortedMap containing a list of
     * the positions and their respective times. For example if a node hit the
     * left wall once, this function would return a map with two elements, one
     * of the collision with the wall itself and then another of the resting
     * point of the node after the reflection.
     */
